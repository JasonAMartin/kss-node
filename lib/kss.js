...
data.rawHTML = currSection;

/*console.log("raw:::::", data.rawHTML);
*/



//console.log("curr sel::::", currSection.data.reference);
		 data.sections.push(currSection);

		 data.sections.sort(function(a, b) {
  return compareStrings(a.data.reference, b.data.reference);
});

	//	console.log("data sections push:::::", data.sections);
...
checkReference = function(paragraphs, options) {
	...
	if (words && words[0].toLowerCase() === 'styleguide') {
		numbers = paragraph.match(/styleguide\s*([a-zA-Z0-9\.]*)/i);
		if (numbers[1]) {
